...
// Automated DataFlash-log analysis (modern stand-in for upstream Utilities/LogAnalyzer + py2exe
// test runner). Each test reads real fields via DataFlashLog.ReadField and returns GOOD/WARN/FAIL,
// mirroring the upstream test categories (Vibration, GPS, VCC, Compass, Power, Motor balance, NaN…).
...
public static class LogAnalyzer {
  // Pure threshold classifier (unit-tested). higherWorse: larger value = worse.
  ...
  private static LogTestResult TestVibration(string bin) {
    ...
    // ArduPilot caution ~30 m/s/s, danger ~60.
  ...
  private static LogTestResult TestVcc(string bin) {
    ...
    double min = vcc.Min();
    double max = vcc.Max();
    // Board 5V rail: warn if it sags below 4.6 or the rail moves more than 0.3V.
  ...
  private static LogTestResult TestCompass(string bin) {
    ...
    // Large offsets indicate poor calibration / interference.
  ...
  private static LogTestResult TestMotorBalance(string bin) {
    ...
    // Large average-output spread suggests an unbalanced airframe / CoG.
