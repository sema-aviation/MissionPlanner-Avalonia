...
// Horizontal Situation Indicator — faithful port of MissionPlanner Controls/HSI.cs.
// A compass rose rotates under a fixed aircraft symbol; an orange heading-bug marks the
// nav bearing (course). Fed by cs.yaw (Heading) and cs.nav_bearing (NavHeading).
public class Hsi : Control {
  ...
  private static readonly Pen WhitePen = new(Brushes.White, 1);
  ...
  public override void Render(DrawingContext ctx) {
    ...
    // ---- rotating compass rose (rotate by -Heading, mirrors HSI.cs) ----
    using (ctx.PushTransform(Matrix.CreateRotation(-Heading * Math.PI / 180.0, c))) {
      for (int a = 0; a < 360; a += 5) {
        double rad = a * Math.PI / 180.0;
        // upstream draws ticks pointing "down" then rotates; here we point outward from centre.
    ...
    // ---- fixed aircraft symbol (orange), mirrors HSI.cs ----
    double s = size / 200.0;
    ...
    // ---- centre readouts: heading + distance to next waypoint ----
